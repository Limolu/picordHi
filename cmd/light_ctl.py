...
class LightControl(commands.Cog):
    def __init__(self, bot):
        ...
        self.SCHEDULE_FILE = './data/light_schedule.json'
    ...
    @app_commands.command(name="light_on", description="設置開燈定時")
    @app_commands.describe(time="設置時間 (HH:MM)", task_id="定時任務ID")
    async def light_on(self, interaction: Interaction, time: str, task_id: str):
        try:
    ...
    @app_commands.command(name="light_off", description="設置關燈定時")
    @app_commands.describe(time="設置時間 (HH:MM)", task_id="定時任務ID")
    async def light_off(self, interaction: Interaction, time: str, task_id: str):
        try:
            time_obj = datetime.strptime(time, '%H:%M').time()
            if any(task['id'] == task_id for task in self.schedule):
                await interaction.response.send_message(f"ID '{task_id}' 已被使用，請換一個ID")
